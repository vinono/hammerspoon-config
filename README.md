...
- **工作机制**：
  - **事件驱动监听**：利用 `hs.pasteboard.watcher` 挂载系统事件回调进行异步监控。抛弃了 Lua 层的定时器轮询，使常态 CPU 占用降为 0%，且响应延迟由 1.0s 缩短至 0.25s 内，极其敏捷。
  - **智能过滤**：自动过滤并去重已存在的记录，防止堆积；限制单条最大长度为 100KB（超过则忽略），避免内存和持久化 I/O 开销骤增；采用 Lua 的 `utf8` 库对超长文本进行 50 字符安全截断，防止中文字符乱码，并替换换行符为 ` ↵ ` 以保护菜单结构。
  - **复制反馈**：点击任意历史条目可将其写回系统剪贴板，并通过轻量气泡 `已复制到剪贴板 ✅` 给予反馈，但默认**不执行自动粘贴**（避免污染当前输入焦点）。
  - **持久化**：所有的历史条目均利用 `hs.settings` 实现本地同步存储。即便重启软件或自动重载，历史记录也绝不丢失。
  - **一键清空**：在菜单列表底部提供 `🗑️ 清空历史` 选项，一键抹除所有缓存。
