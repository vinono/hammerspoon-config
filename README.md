...
>
> 这是一个高度精炼、稳健且模块化的 Hammerspoon 配置项目。致力于解决 macOS 系统在日式键盘（JIS Layout）输入法切换时的痛点，并提供便捷的系统常亮控制（Caffeine）与豆包语音结束后的输入法恢复。
...
| **`ime/doubao_return`** | **豆包语音回切** | 豆包全局语音结束后，恢复语音开始前的 ABC 或系统拼音输入法。 | - |
...
│   └── doubao_return.lua # 豆包全局语音结束后恢复原系统输入法
├── caffeine/
...
### 3. 豆包全局语音回切
豆包全局语音会临时接管输入法。模块会记录语音前稳定的 ABC 或系统拼音输入源；确认语音浮窗出现并关闭后，等待 2 秒再恢复原输入法，避免把触发过程中的中间输入法状态误当作目标。
